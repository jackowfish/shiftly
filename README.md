...
Off by default, under Eye Tracking (Beta) in the menu bar item. It doesn't add a gesture. It changes which window the gestures above act on: hold your usual modifier, look at the window you want, and the next arrow press grabs that one instead of whatever happened to be frontmost. The window never has to be focused first.
...
It only ever picks the window on top at that point. Whatever is drawn over the spot you're looking at is the thing you can see there, so a window behind it is one you're provably not looking at, and it isn't a candidate however well the estimate fits it.
...
So halves are comfortable, thirds are fine on a wide screen, and sixths is the edge of what this can do. The "does nothing" column is large by design: those are presses that fall back to normal focus behaviour, which is the failure you don't notice.

Two things worth reading off the table. A slot only has to beat the *other windows actually open*, so two or three windows on a screen is a much easier problem than a fully tiled one, and that's the case it handles well. And what matters is the size of the slot rather than the layout it came from: a 564px-wide third of the portrait display does worse than a quarter of the ultrawide at three times the width, and the two sixths rows differ by the same logic.

Widening the "properly inside" margin doesn't rescue the small slots. Sweeping it from 80px to 300px gets the ultrawide under 5% wrong only by turning about 69% of presses into no-ops, and the portrait display floors at 8.3% because the margin is capped at a fifth of the slot. So eye tracking is best thought of as a way to grab one of the two or three windows you actually have open, and gestures still work exactly as before when it declines.
